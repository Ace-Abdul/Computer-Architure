...
int main(int argc, char * argv[]){
    ...
    int frames = cacheSize/blockSize;
    int sets = cacheSize/blockSize/ways;
    int indexBits = ln2(sets);

    struct dataBlock *** cache = (struct dataBlock *** ) malloc(sizeof(struct dataBlock **)); 
    ...
    while(fscanf(f, "%s", inst)!=EOF){
        ...
        if(inst[0] == 'l'){   // Load instruction
            ...
            if (goMemory){
                ...
                struct dataBlock * neew = (struct dataBlock *) malloc(sizeof(struct dataBlock)); //Create a pointer to a new instance of pizzaStruct
                neew->myState = 1;
                neew->myTag = tag;

                for(int n = 0; n < blockSize; n++) {
                    neew->byteData[n] = memory[address-blockOffset+n]; 
                }
